...
* Eclipse will say it doesn't know what those types are.  You have to mouse-over the offensive code and allow eclipse to suggest what you should do.
...
In this exercise, you use IntelliJ to *auto-generate* useful parts of a class.  Try this on your own, but as you need help, run the solution video only as far as necessary for you to move forward again on your own.
...
3. Auto-generate the constructor by choosing `Code`, then `Generate...`, then select `Constructor` from the pop-up menu.
...
6. Now use IntelliJ to generate the *getters* for `x` and `y`.  
...
8. Use IntelliJ similarly to generate a simple `toString()` method.
...
10. Now let\'s use IntelliJ to generate `hashCode()` and `equals(Object other)` methods.
