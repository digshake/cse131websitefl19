...
3. Open `readEntitiesTest.java` and run it.  You can run all the tests in the file by clicking the "play" button next to the class name:<br/>![Run All Tests](../../../assignments/RunAllTests.png) <br/>
...
7. Compete the code for the `touchingZombie` method.  Again, notice that the comments above it give a brief description what it should do.
   * Hint: Consider entities to be touching if they overlap. Use the distance formula.
8. Open `touchingZombiesTests.java` and run all the tests.
   * Update your `touchingZombies()` method until it passes all tests.
9.  Return to `ZombieSimulator.java` and complete the code in `updateEntities()`. Again, follow the directions given in the comment.
    ...
    * If a non-zombie touches a zombie it will become a zombie (i.e., it's type will change.)
...
12. Complete the code for `survivorCount()`
13. Open `survivorCountTests.java`
...
16. *Review your work!*  Make sure you understand the info used by each method and what each returns.  Review how the individual methods fit into the overall process of simulating the zombie world.
17. Feel free to add in more features:
