...
[Refactoring](https://en.wikipedia.org/wiki/Code_refactoring){:target="_blank"} is a technique used to refine computer code.  The basic idea is to restructure code without changing overall behavior.  Often refactoring is done because either: a) code is difficult to manage (too complex) and reorganizing it will make it easier to update/fix/understand or b) new techniques provide better ways of writing the code.  Part of your work here will be easier if you just refactor some of your work from Lab 4.
...
int b = anArray[i][TYPE];     // and this column is indexed with "TYPE"
~~~
...
Whenever you sue the concept of `ZOMBIE`, `NONZOMBIE`, `X`, `Y`, and `TYPE` in in this program use those variables, *not* the corresponding number.
...
If you fail any unit test cases you should try to read through the test case and see what it's testing.  In this lab all tests cases are either looking for a particular value (via `assertEquals()`) or for a condition to be true (`assertTrue()`).  Most test cases use concepts that you've encountered previously in the class.  With a little review you can probably figure out what the test case is expecting. 

# Procedure & Recommended Workflow

Below is a recommended work-flow.  Many of the parts are independent and if you get stuck on one you can leave it incomplete and move on to the next, but you may need to ensure that methods return a value and that there aren't syntax errors.

1. Open the `ZombieSimulation` in the `src` folder.  Some items have already been done for you.  Most of the remaining work is labeled in the file with `TODO` comments.
...
2. Open `readEntitiesTest.java` and run it.  You can run all the tests in the file by clicking the "play" button next to the class name:<br/>![Run All Tests](../../../assignments/RunAllTests.png) <br/>
3. Examine the results of the test cases.  If they all passed you'll see the following in IntelliJ: <br/>![Read Tests](../../../assignments/ReadTestsPassed.png) <br/>
   * If a test failed it will be displayed in red with an X rather than a check mark.  You can look at the test case to try to identify what it's testing and then try to update your code to fix the problem.
   * Update your `readEntities()` method until it passes all tests.
4. Return to `ZombieSimulator.java` and complete `drawEntities()`.  Notice that part of it has already been done for you. Again, you can merge in code from Lab 4.  Review the comment before `drawEntities()` and make sure your code works as described. (The comment is in a special format called JavaDoc)
5. Open `drawEntitiesTests.java` and run the unit tests.  
   ...
   * Update your `drawEntities()` method until it passes all tests.
6. Compete the code for the `touchingZombie` method.  Again, notice that the comments above it give a brief description what it should do.
7. Open `touchingZombiesTests.java` and run all the tests.
   * Update your `touchingZombies()` method until it passes all tests.
8. Return to `ZombieSimulator.java` and complete the code in `updateEntities()`. Again, follow the directions given in the comment.
9. Open `updateEntitiesTests.java` and run the tests.
    * Update your `updateEntities()` method until it passes all tests.  
10. Return to `ZombieSimulator.java`.  You'll need to complete a `survivorCount()` method. Search the file for `survivorCount`.  The file already contains the comment block, but the method itself is commented out because it is incomplete.  Remove the comments and replace the two `TYPE` words with appropriate types for the return value and parameter type.  
11. Complete the code for `survivorCount()`
12. Open `survivorCountTests.java`
    * All the tests were commented out until `survivorCount()` was completed.  Remove the comments and run the tests.
    * Update your `survivorCount()` method until it passes all tests.
13. Almost done!  You just need to complete the actual simulation. Open `ZombieSimulator.java` and complete the `TODO` in the `main()` method. HINT: this should be short.  It can be done in ~4-6 lines of code.
14. Run `ZombieSimulator.java`.  Try a few of the files that are provided (`zombieWorld.sim`, `surrounded.sim`, `inTheHouse.sim`, `bubbles.sim`, etc.)
15. *Review your work!*  Make sure you understand the info used by each method and what each returns.  Review how the individual methods fit into the overall process of simulating the zombie world.
16. Feel free to add in more features:
...
Partial credit is possible, but for full credit:
